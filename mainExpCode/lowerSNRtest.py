...
def keyCheck(keyList, win, clock, logfile,  catchStart, rt, response_key,end_trial):
    ...
    if not keys == []:
        ...
        print(f'Response key is {response_key}\n' )
    elif not keys == []:
        escape_check(keys,win,logfile)
...
#%% =============================================================================
# open log file
dataName = f'{logname}_SNRtest_{exp_info["date"]}.csv'
...
for instnr in instructiontexts:
    ...
    keys = event.waitKeys(keyList=keyList)#core.wait(.1)
    escape_check(keys,win,logfile)
    #win.close()
#############################################################test button box
...
optimal_signal(acclist_per_signal, 85) ################################################### signal closest to 85% correct
