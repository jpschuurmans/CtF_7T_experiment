...
def bentontask(base_path,subject):
    datapath = f'{base_path}data/'  # directory to save data in     
    Fpath= f'{base_path}benton_faces/' # directory where images can be found
    ...
    data_fname = f"{datapath}{subject}_benton.csv"
    ...
    bitmap8 = visual.ImageStim(win, size=[35, 20])
    ...
    for instruc in cons[0:2]:
        cons_imname=f'{Fpath}consignes{instruc}.bmp'
    ...
    for Trial in first:
        ...
        face1_imname=f'{Fpath}r{numbers[Trial0]}A.bmp'
        bitmap1.setImage(face1_imname)
        bitmap1.pos=(-6, -0.5)
        face2_imname=f'{Fpath}r{numbers[Trial0]}B.bmp'
        bitmap2.setImage(face2_imname)
        bitmap2.pos=(0, -0.5)
        face3_imname=f'{Fpath}r{numbers[Trial0]}C.bmp'
        bitmap3.setImage(face3_imname)
        bitmap3.pos=(6, -0.5)
        face4_imname=f'{Fpath}r{numbers[Trial0]}D.bmp'
        bitmap4.setImage(face4_imname)
        bitmap4.pos=(-6, -6.5)
        face5_imname=f'{Fpath}r{numbers[Trial0]}E.bmp'
        bitmap5.setImage(face5_imname)
        bitmap5.pos=(0, -6.5)
        face6_imname=f'{Fpath}r{numbers[Trial0]}F.bmp'
        bitmap6.pos=(6, -6.5)
        bitmap6.setImage(face6_imname)
        face7_imname=f'{Fpath}s{numbers[Trial0]}_cropped.bmp'
    ...
    cons_imname=f'{Fpath}consignes4.bmp'
    bitmap8.setImage(cons_imname)
    bitmap8.pos=(0, 0)
    bitmap8.draw()
    win.flip() #fliping the screen to show images
    ...
    cons_imname=f'{Fpath}consignes5.bmp'
    bitmap8.setImage(cons_imname)
    bitmap8.pos=(0, 0)
    bitmap8.draw()
    win.flip() #fliping the screen to show images
    ...
    for Trial in third:
        ...
        core.wait(1)
    print ('score=', score)
    cons_imname=f'{Fpath}consignes6.bmp'
    bitmap8.setImage(cons_imname)
    bitmap8.pos=(0, 0)
    bitmap8.draw()
    win.flip() #fliping the screen to show images
